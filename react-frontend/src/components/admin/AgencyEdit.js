...
const AgencyEdit = (props) => {
    ...
    useEffect(() => {
        fetch(`https://localhost:44398/api/AgencyService/Agency/${userID}`).then(
            (response) => {
    ...
    const editProfile = async (e) => {
        ...
        const res = await axios.put('https://localhost:44398/api/AgencyService/EditAgency',
          {id:userID,name:name, phone: phone, address: address,
            email: email, nid: nid, password:userData.password});
          if(res.status === 200){
            toast.success("Agency Edit Successful");
              
          }if(res.status === 204){
            toast.success("Agency Edit Successful");
        }
          else{
            toast.error("An error occured to while editing a agency");
          }
